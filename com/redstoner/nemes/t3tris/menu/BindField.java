...
public class BindField extends Button {
	...
	private EnumKeyBind control;
	private boolean hovering = false;
	
	public BindField(float x, float y, float x2, float y2, IMenu m, EnumKeyBind c) {
		super(x, y, x2, y2, m);
		control = c;
	}
		...
		if (x > x_ && x < x2_ && y > y_ && y < y2_) {
			...
			if (Keyboard.next()) {
				if (Keyboard.getEventKeyState()) {
					EnumKeyBind.setKeyCode(control, Keyboard.getEventKey());
				 }
			 }
		}else{
			hovering = false;
