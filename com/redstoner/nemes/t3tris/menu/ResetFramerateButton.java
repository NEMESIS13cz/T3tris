...
public class ResetFramerateButton extends Button {
	...
	public void render(int w, int h) {
		...
		FontMap.drawString(x_ + (x2_ - x_) / 2, y_ + (button_h - scale * 8) / 2, scale, "Reset framerate limit", Constants.BUTTON_TEXT_COLOR, true);
	}
	...
	public void update(int x, int y, int w, int h) {
		...
		if (x > x_ && x < x2_ && y > y_ && y < y2_) {
			if (MouseHandler.released(Controls.getLeftMouseButton())) {
				Options.framerateLimit = Options.DEFAULT_FRAMERATE_LIMIT;
